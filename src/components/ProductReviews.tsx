...
export default function ProductReviews({
  ...
  useEffect(() => {
    ...
    // Delay scroll until the animated container has mounted and expanded.
  ...
  if (isLoading) {
    return (
      <div className="animate-pulse space-y-5 sm:space-y-6">
        ...
        <div className={reviews.length === 0 ? 'space-y-4' : 'grid gap-4 lg:grid-cols-2'}>
          {[1, 2, 3].map((item) => (
      ...
      <AnimatePresence initial={false}>
        {showReviewForm && (
          ...
          >
            <div
              ref={reviewFormRef}
              className="rounded-[28px] border border-green-100 bg-white p-5 shadow-sm sm:p-6"
            >
